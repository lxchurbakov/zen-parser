...
const hasReduces = (machine) => machine.table[machine.stack.last().state].reduces.length > 0;
...
const burnMachine = (machine) => {
  ...
  reduces.forEach(reduce => {
    find(rules, rule => rule.name === reduce.ruleName).resolve(rule => {
      ...
      const _rest = newMachine.stack.splice(-rule.pattern.length);
      const rest  = _rest.map(v => v.value);

      const newTapeElement = rule.map ? rule.map(rest) : { token: rule.production, rest };

      newMachine.tape.unshift(newTapeElement);
  ...
  shifts.forEach(shift => {
    ...
    const value = newMachine.tape.shift();

    const newStackEntry = { state: shift.index, value };

    newMachine.stack.push(newStackEntry);
    newMachine.weight.b = shift.priority + (shift.side === 'left' ? -0.5 : 0.5);

    if (value && shift.on === value.token) {
      result.push(newMachine);
    }
  });
