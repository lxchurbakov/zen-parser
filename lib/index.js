...
const wasSetParsed = (unparsedSet, parsedSets) => {
  debugger;
  return parsedSets.find(parsedSet => isUnparsedSetParsed(unparsedSet, parsedSet));
}
...
while(unparsedSets.length > 0) {
  ...
  if (parsedIndex !== -1) {
    ...
    // Current set repeats already parsed one..
    ...
    Object.keys(reduces).forEach(name => {
      ...
      table[currentSet.index]['all'] = { type: 'reduce', rule: reduces[name].ruleName, priority: reduces[name].priority, side: reduces[name].side };
    });
    ...
    Object.keys(shifts).forEach(name => {
      ...
      table[currentSet.index][name] = { type: 'shift', index: newSet.index, priority: shifts[name].priority, side: shifts[name].side };
    });
